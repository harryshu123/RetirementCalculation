...
public class Retirement {
	...
	public double MonthlySavings() {
		...
		double pv = this.TotalAmountToSave();
		double r = dAnnualReturnWorking/12;
		int month = iYearsToWork*12;
		double up = pv*r;
		double under = 1 - Math.pow(1+r, -month);
		pmt = up/under;
	...
	public double TotalAmountToSave() {
		...
		double pv = 0;
		double under = Math.pow((1 + dAnnualReturnRetired/12),(double)(iYearsRetired *12));
		double pmt = dRequiredIncome - dMonthlySSI;
		double rate = Math.pow((1 + dAnnualReturnRetired/12),(double)(iYearsRetired *12));
		double smallrate = dAnnualReturnRetired/12;
		double up = pmt * ((rate-1)/smallrate);
		pv = up/under;
